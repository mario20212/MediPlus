...
class Schedule {
    ...
    static async checkAndSendReminder() {
        ...
        // Check if the current hour matches any of the specified options in whenToTakeValues
        allSchedules.forEach(schedule => {
        try{ 
            if (schedule.whenToTakeValues == "1beforeBreakfast"  && currentHour === 1 && currentMinutes === 37) {
    ...
    static async continuousReminderCheck() {
        while (true) {
            ...
            await new Promise(resolve => setTimeout(resolve, 1000)); // Delay for one second
        }
    }
}
